...
const viewCastButton = document.createElement("button");
viewCastButton.className = "btn btn-danger"; // Add Bootstrap button classes
viewCastButton.innerText = "View Cast";
viewCastButton.onclick = function (event) {
  if (!link.show.id) {
    return alert('No cast found!');
  }
  window.location.href = `view-cast.html?id=${encodeURIComponent(link.show.id)}`;
};

const viewCrewButton = document.createElement("button");
viewCrewButton.className = "btn btn-success"; // Add Bootstrap button classes
viewCrewButton.innerText = "View Crew";
viewCrewButton.onclick = function (event) {
  if (!link.show.id) {
    return alert('No crew found!');
  }
  window.location.href = `view-crew.html?id=${encodeURIComponent(link.show.id)}`;
};

// Append the button to the showInfoDiv
showInfoDiv.appendChild(viewCastButton);
resultsContainer.appendChild(showInfoDiv);

showInfoDiv.appendChild(viewCrewButton);
resultsContainer.appendChild(showInfoDiv);
